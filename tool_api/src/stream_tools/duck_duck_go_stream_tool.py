...
def get_display_str(results):
    combined_str = "\n"
    for res in results:
        combined_str += f"- [{res['title']}]({res['href']})\n"
    return combined_str
...
class DuckDuckGoSearch(Tool):
    def run(self, tool_input, max_results=10):
        ...
        yield "display", get_display_str(results)
        relevant_results = mp_filter_results(tool_input, results)
        yield "result", str(json.dumps(relevant_results, indent=4))
